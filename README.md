...
📌 **기본 스킬 = 각인 규약**: `WeaponData.skills` 4개가 그대로 각인으로도 등록된다(Sword `Sword_Skill_00~03`, Dagger `Dagger_Skill_00~03`). Dagger는 2026-08-03에 `SkillData` → `EngravingData` 전환으로 편입했다 — `EngravingData`가 `SkillData` 상속에 `owningForm`+`grade`만 추가하므로 **`.asset`의 `m_Script` guid 교체만으로 전환**되고, **`.asset.meta`를 건드리지 않으면 `WeaponData.skills` 결선이 자동 유지**된다.
...
| **상태이상 과부하** | 상태이상 | 독·출혈이 **프로필 최대 스택 도달** 시 **전량 소모** + 즉발 피해. 출혈은 추가로 **피해 비율만큼 회복**. 단일 대상, 전파 없음 | 보너스 30% → `DamagePerStack × N(N+1)/2 × 1.3` / 출혈 회복 40% (Poison 엔트리는 0) |
...
- **과부하 공식 선택 근거**: 전량 소모형이라 "틱뎀 × 배율" 방식은 배율이 남은 틱 수를 못 넘으면 순손실이 된다(지속 5초·틱 1초면 5배가 본전). **잔여 총량 기준**으로 바꾸면 항상 100% 회수 + 보너스가 덤이라 **손익분기 개념 자체가 소멸**하고 보너스율만 튜닝 대상으로 남는다.
- **타입별 분화(2026-08-03)**: 초기 안은 독 전용이었으나 **정작 Dagger 킷이 출혈을 더 많이 걸어** 자주 거는 쪽이 지원을 못 받는 역전이 있었다. 임계를 **프로필 캡**으로 재정의해 두 타입을 자동 편입시킨 뒤, `ailmentOverloadHealPct`를 **엔트리 필드**로 두어 Bleed에만 40을 저작했다. **타입 분기 코드가 하나도 없다** — Poison 엔트리는 값이 0이라 자연히 회복이 없고, 상태이상이 늘어도 코드 변경 없이 저작만으로 확장된다.
- **회복이 필요했던 근거**: Dagger 3축 중 **상태이상 축만 흡혈 환산이 0**이었다. 표식폭발 proc은 광역 피해가 전량 흡혈로 환산되고(§7-13 `IsProcCast` 게이트 밖에 `ReportLifestealDamage`가 있다) 처형은 램프 증폭분이 환산되는데, 과부하는 DoT 틱 콜백을 재사용해 흡혈 소스 목록에 없다. 회복은 **자체 계수**로 처리하고 `ReportLifestealDamage`를 경유하지 않는다 — 흡혈 배관을 타면 소울 `Lifesteal` 미투자 시 0이 되어 "소울 미투자 죽은 드랍" 원칙 위반. 과회복 쉴드 전환은 Sword 생명 흡수와 중복이라 미채택(풀피에서 새는 것은 "체력 낮을수록 가치↑" 리듬으로 의도된 동작).
- 🔒 **유물의 `AilmentOverload` 저작 금지(Dashboard Warning)**: 과부하는 폼선택 1택 근간 축의 **정체성**이라 런 중 획득·교체되는 유물이 부여해서는 안 된다. `AttackPoison`/`AttackBleed`는 봉쇄 대상이 아니다(유물이 독·출혈을 거는 것은 계속 가능). 런타임 차단은 하지 않는다 — `AddPassiveBehavior`가 유물·패시브 공용 진입점이라 물리 차단은 구조 변경이 된다. 📌 **부수 효과**: 유물이 봉쇄되면 `_ailmentOverloads` 리스트가 **런 중 절대 바뀌지 않으므로**(패시브는 런 중 변경 불가) `CombatEffectContext`가 라이브 참조를 들고 다녀도 안전하다. 투사체·장판은 컨텍스트를 품고 수 초간 유지되므로, 유물 저작이 허용되면 날아가는 도중에 조건이 바뀔 수 있었다.
...
- 결의의 받는 피해는 **`incoming × 2 → 방어 차감 → Max(1) → 쉴드 흡수`** 순서다(2026-08-10 `ee70c2b9`로 곱셈을 방어 앞으로 이동). 🔑 **감산인 방어가 곱셈 안쪽에 있으면 방어 계수까지 배율을 먹는다** — 이전 순서(`(incoming − 방어) × 2`)에서는 방어 1점이 최종 피해 2점을 막아, "받는 피해만 2배"라는 의도와 어긋났다. **쉴드는 곱셈 밖의 풀이므로 이동 대상이 아니다**(`Absorb`가 잔여를 반환 → 1점당 최종 피해 1점, HP와 동일 축). 유효 체력이 반감되는 것이 페널티의 정의 그 자체이고, 쉴드를 곱셈 앞으로 옮기면 결의+쉴드가 명시적 시너지 축이 된다 — 버그 수정이 아니라 기획 선택이라 하지 않았다
...
**`_version` 가드(2026-07-10 도입, 전환 후에도 유지)**: 콜백 중 `Clear()`가 발생하면 로컬 버킷 write-back이 이를 덮는 문제 — Clear마다 버전 증가, 콜백 후 불일치면 write-back 중단. **틱 경로와 과부하 폭발 경로 양쪽에 필요**(폭발이 적을 죽이는 것은 정상 경로이며, 그 안에서 `Die()`→`ResetStatusEffects()`→`Clear()`가 돈다). ⚠️ **과부하 회복은 이 검사 앞에서 호출해야 한다**(§7-8b).

**스택 누적의 성립 조건(2026-08-03 도출)**: DoT 수명 = `현재 스택 × 프로필 틱 간격`이므로 출혈 1스택은 0.5초 만에 소멸한다.

```
부여 간격 < 현재 DoT 수명  →  누적 시작
부여 간격 > 현재 DoT 수명  →  영원히 1스택
```

**쿨다운이 있는 단발 스킬로는 캡에 도달할 수 없다** — 2스택을 걸어도 수명 1초인데 쿨이 3초면 그 사이 사라진다. 누적이 가능한 형태는 **장판(§7-12) 또는 다단 히트뿐**이며, 그 외 스킬의 상태이상은 "장판 안에서 밀어넣는 보조"로만 의미를 갖는다. 상태이상 축 패시브(과부하)가 성립하려면 킷에 장판·다단이 최소 하나 있어야 한다는 요구가 여기서 나온다.

**과부하 연동**: 장판은 틱마다 `Apply`를 호출하므로 **캡 도달 → 폭발 → 버킷 비움 → 재축적 → 재폭발**이 지속 동안 반복된다. 장판의 틱 간격·부여 스택·지속을 정하는 것이 곧 폭발 횟수를 정하는 것이다.

**콘솔 로그 공백**: 과부하 폭발은 `_applyTickDamage`(DoT 틱 콜백)를 재사용하므로 **`LogDamageDealt`를 타지 않아 피해값이 콘솔에 안 찍힌다.** 회복량도 로그가 없다. 수치 검증이 필요하면 `[Conditional("UNITY_EDITOR")]` 진단 로그를 임시로 넣는 것이 가장 빠르다(2026-08-03에 회복 실측 검증에 사용 후 제거).
...
🔑 **상태이상 장판 저작 공식(2026-08-03)** — DoT가 스택 소모형이라 **부여와 동시에 차감이 진행**된다.

```
순증율 = (부여 스택 ÷ zoneTickInterval) − (1 ÷ 프로필 tickInterval)
캡 도달 시간 ≈ 캡 ÷ 순증율
```

**순증율이 0 이하면 어떤 지속을 줘도 폭발하지 않는다.** 혈흔 첫 저작(`stacks 1` / 틱 0.5s)이 정확히 이 경우로, 부여 초당 2 = 소모 초당 2라 1~0 스택을 왕복했다. `stacks: 2`로 순증 2/초를 확보해 캡 5까지 약 1.5초.

⚠️ **독은 더 엄격하다** — 프로필 틱 1.0s라 소모가 초당 1인데 `stacks 1` / 틱 0.5s면 순증 1/초 → 캡 10까지 10초로 사실상 불가능하다. **stacks 2 이상 필요.**

⚠️ **이름이 겹치는 두 틱 간격**: `SkillData.zoneTickInterval`(상태이상 **부여** 빈도) vs `EnemyAilmentProfileDatabase` 프로필 `tickInterval`(DoT **피해** 빈도). 캡 도달 속도를 정하는 건 앞의 것이다.

⚠️ **`damage`는 0과 1 사이에 중간값이 없다** — `tickDamage = TotalAttack + skillDamage`라 1만 넣어도 **공격력이 통째로 틱 수만큼 곱해진다**(11틱이면 11배). "낮은 딜 장판"은 저작으로 표현 불가능하므로, 상태이상 전용 장판은 `damage: 0`으로 두고 순수 부여 경로를 탄다.

⚠️ **`ZonePayload.HasAnyEffect`** = `TickDamage > 0 || Slow > 0 || Ailments 존재`. `damage: 0` 장판은 **ailments가 유일한 통과 근거**이므로 비우면 `"ZonePayload has no effect"` 경고와 함께 생성 자체가 실패한다.
